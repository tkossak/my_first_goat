...
class MyFirstGoat:
    ...
    def __init__(self,
        ...
        self.discord_user_debug: Union[discord.User, None] = discord.User(id=self.discord_user_debug_id)
        self.discord_user_send: Union[discord.User, None] = discord.User(id=self.discord_user_send_id)
    ...
    def setup_config(self) -> bool:
        ...
        if MyFirstGoat.config_home.exists(config_file_name):
            ...
            config_template_file_path = Path(__file__).parent / 'data' / config_template_file_name
            config_str = config_template_file_path.read_text()
            MyFirstGoat.config_home.settext(
                config_file_name,
                config_str
            )
            print(f'Config data was copied to {MyFirstGoat.config_home.getsyspath(config_file_name)}')
    ...
    async def bot_send_loot_messages(self):
        ...
        hw_words_plus_1_minus_1 = [
            'each',
            'split',
            'splits',
        ]
        hw_words_plus_1_the_end = [
            'so',
        ]
        ...
        def get_how_much_per_person(msg_list):
            ...
            # check only +1:
            val = check_how_much_plus_n(msg_lower=msg_lower,
                                        searched_words=hw_words_plus_1,
                                        n=1)
            if val:
                return val
            ...
            # check only -1:
            val = check_how_much_plus_n(msg_lower=msg_lower,
                                        searched_words=hw_words_minus_1,
                                        n=-1)
            if val:
                return val
            ...
            # check +1 then -1:
            val = check_how_much_plus_n(msg_lower=msg_lower,
                                        searched_words=hw_words_plus_1_minus_1,
                                        n=1)
            if val:
                return val
            val = check_how_much_plus_n(msg_lower=msg_lower,
            ...
            val = check_how_much_plus_n(msg_lower=msg_lower,
                                        searched_words=hw_words_plus_1_the_end,
                                        n=1)
            if val:
                return val
        ...
        async for msg in self.client.logs_from(
            ...
            for creditor_obj in msg.mentions:
                ...
                if i+1 < len(msgl) and msgl[i+1].lower() in paid_strings:
                    ...
                    # NOT paid
                    debtors[msg.author].append(Entry(creditor=creditor_obj.mention,
                                                     how_much=how_much_per_person,
                                                     created=msg.timestamp))
        log.info(f'Oldest message: ' + str(min(d.created for key, val in debtors.items() for d in val)))
        ...
        for single_msg in msgs:
            if self.cli_test:
                await self.client.send_message(destination=self.discord_user_debug, content=single_msg)
            else:
                await self.client.send_message(destination=channel_general, content=single_msg)
    ...
    async def bot_download_and_save_members_in_db(self):
        ...
        for m in self.members:
            rows_to_insert.append((cur_epoch_time, self.server.id, self.server.name, m.id, m.name, m.display_name))

        # Insert list to DB:
        self.cur.executemany('INSERT INTO members VALUES(?, ?, ? ,?, ?, ?)', rows_to_insert)
        self.con.commit()
    ...
    async def bot_send_members_status(self):
        ...
        await self.client.send_message(
            destination=self.discord_user_send,
            content=self.text_full
        )
    ...
    def compare_snapshots_and_prepare_msg(self):
        ...
        self.cur.execute('select max(datetime) from members where datetime != ?', (day_b,))
        ...
        for r in self.cur.execute(sql, dict(day_a=day_a, day_b=day_b)):
            ...
            if user['jest_a'] and user['jest_b'] and user['display_name_a'] != user['display_name_b']:
        ...
        all_current_members = sorted(all_current_members, key=lambda x: x['display_name_b'].lower())
        renamed_members = sorted(renamed_members, key=lambda x: x['display_name_b'].lower())
        ...
        **CURRENT MEMBER STATUS:**
        ...
        max_old_name_length = max(len(m['display_name_a']) for m in renamed_members) if renamed_members else 0
        ...
        ren_members_txt = self.text_codify_for_discord(
            '\n'.join(
                f"{m['display_name_a'].ljust(max_old_name_length)} => {m['display_name_b']}" for m in renamed_members
            ) if renamed_members else ''
        )
        all_members_txt = self.text_codify_for_discord(
            '\n'.join(m['display_name_b'] for m in all_current_members) if all_current_members else ''
        )
        text = text.format(
